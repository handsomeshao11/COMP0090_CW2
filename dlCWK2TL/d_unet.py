# Task script for the D-Unet model
...
if __name__ == "__main__":
    ...
    ### Build and fit the D-Unet model, with parameters fixed after pre-training
    ...
    # Set up the  D-Unet model
    model_name = 'dunet'
    setting = ['pretrain', 'non-ft']
    model = d_unet_model(img_shape, pre_train='imagenet', fine_tuning=False)
    model.compile(optimizer=Adam(lr=1e-5), 
                  loss=dice_coef_loss, 
                  metrics=[iou, dice_coef, 'binary_accuracy'])

    # Add models folder to save models, if folder does not already exist
    if os.path.exists(get_path('models')) == False:
